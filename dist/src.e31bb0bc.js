...
var codeRows;
var consoleRows;
var areas = ["code", "console", "print", "about"];
var tabs = ["code", "console", "print", "about"].map(function (area) {
  return setTab("");
});
...
var tabAbout = setTab($("#tab-about"), $("#about-section")); //GETTING JSON

var xhr = new XMLHttpRequest();
xhr.open("GET", "json/keyContent.json", true);
xhr.send();
var keyContent = xhr.response; //START set
...
setInputDefault($("#console-textarea")); //CHECKBOXES
...
$("#btn-code-send").click(function () {
  var thead = $("thead th").empty();
  var caption = $("#code-caption").empty();
  var tbody = $("#table-body-rows").empty();
  thead.text($("#code-input-header").val());
  caption.text($("#code-input-caption").val());

  for (var i = 0; i < codeRows.length; i++) {
    for (var content in keyContent.key) {
      if (codeRows[i].includes(keyContent.key[content])) {
        var string = codeRows[i].match(keyContent.key[content]);
...
$("#btn-console-send").click(function () {
  var consoleReturn = $("#console-return").empty();
  var caption = $("#console-caption").empty();
...
$("#btn-print").click(function () {
  ...
  $("body").fadeOut();
  $("body").fadeIn();
  (0, _html2canvas.default)(document.querySelector("#print-view")).then(function (canvas) {
    document.getElementById("print-result").appendChild(canvas);
  });
}); //TAB FUCTIONS

function setTab(tabElement, sectionElement) {
  var tab = {
...
},{"html2canvas":"../node_modules/html2canvas/dist/html2canvas.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
